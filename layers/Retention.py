...
class MultiScaleRetention(nn.Module):
    ...
    def forward(self, X):
        ...
        Y = torch.cat(Y, dim=2)  # [B, L, heads * v_dim]
        Y_shape = Y.shape
        Y = self.group_norm(Y.reshape(-1, self.v_dim)).reshape(Y_shape)

        return (self.swish(X @ self.W_G) * Y) @ self.W_O  # [B, L, d_model]

    # forward_recurrent and forward_chunkwise can be similarly adjusted if needed
